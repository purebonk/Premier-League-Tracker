 ...
 * view. This is the shared foundation of the table, form and streak queries:
 * `matches` stores fixtures, but standings are per club, and a lateral join is
...
const CLUB_MATCHES = /* sql */ `
  ...
  ) as c(team_id, goals_for, goals_against)
...
/**
 * League table with full tiebreakers: points, then goal difference, then goals
 * scored, then club name.
 *
 * Three outcomes, not two -- a draw is worth a point to both clubs and is
 * counted separately from wins and losses rather than being inferred.
 *
 * `uptoMatchweek` is what makes the table a time machine: passing null gives
 * the current table, passing N gives the table as it stood once clubs had
 * played N matches each. Note that matchweek here is a games-played index,
 * not the official fixture-list round -- see drizzle/0001_derived_matchweeks.sql.
 */
const LEAGUE_TABLE_SQL = /* sql */ `
  with club_matches as (${CLUB_MATCHES}
  ...
  totals as (
    select
      ...
      -- Three points for a win, one for a draw. Computed rather than stored so
      -- it cannot drift out of sync with the results it is derived from.
  ...
  select
    ...
    team_id, name, short_name, played, won, drawn, lost,
...
export function leagueTable(
  db: Queryable,
  opts: { season: number; uptoMatchweek?: number | null; competition?: string } ,
): Promise<TableRow[]> {
  return db
    .query<Record<string, unknown>>(LEAGUE_TABLE_SQL, [
      opts.competition ?? DEFAULT_COMPETITION,
      opts.season,
      opts.uptoMatchweek ?? null,
    ])
    .then((rows) => rows.map(toTableRow));
}

export interface FormRow extends TableRow {
  /** Most recent first, e.g. "W W D L W". */
  form: string;
}

/**
 * Standings computed over each club's last N matches only.
 *
 * The window is per club, not per date: "last 6 matches" means six matches for
 * every club, even when one of them has a game in hand. That is the number a
 * fan means by form, and it keeps clubs comparable.
 */
const FORM_TABLE_SQL = /* sql */ `
  with club_matches as (${CLUB_MATCHES}),
  recent as (
    select *,
      row_number() over (
  ...
  windowed as (
    select * from recent where recency <= $3::int
  ),
  totals as (
    select
      t.id as team_id,
      ...
      t.short_name,
      count(*)::int as played,
      count(*) filter (where w.outcome = 'W')::int as won,
      count(*) filter (where w.outcome = 'D')::int as drawn,
      count(*) filter (where w.outcome = 'L')::int as lost,
      coalesce(sum(w.goals_for), 0)::int as goals_for,
      coalesce(sum(w.goals_against), 0)::int as goals_against,
      coalesce(sum(w.goals_for) - sum(w.goals_against), 0)::int as goal_difference,
      (count(*) filter (where w.outcome = 'W') * 3
       + count(*) filter (where w.outcome = 'D'))::int as points,
      -- Ordered aggregate: most recent result first.
      string_agg(w.outcome, ' ' order by w.recency) as form
    from windowed w
...
export function formTable(
  db: Queryable,
  opts: { season: number; window?: number; competition?: string },
): Promise<FormRow[]> {
  return db
    .query<Record<string, unknown>>(FORM_TABLE_SQL, [
      opts.competition ?? DEFAULT_COMPETITION,
      opts.season,
      opts.window ?? FORM_WINDOW,
    ])
    .then((rows) => rows.map((r) => ({ ...toTableRow(r), form: String(r.form ?? "") })));
}
